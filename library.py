...
class Library:
    ...
    @staticmethod
    def _track(row: TrackRecord) -> Track:
        return Track(row.id,row.filename,row.title,row.artist,row.album,row.duration,row.has_cover,row.custom_lyrics,bytes(row.audio_data),bytes(row.cover_data))
    def list_tracks(self):
        with SessionLocal() as s:
            rows=s.scalars(select(TrackRecord).where(TrackRecord.user_id==self.user_id)).all()
            return sorted((self._track(r) for r in rows),key=lambda t:(t.artist.lower(),t.title.lower()))
    def get(self, track_id: str):
        with SessionLocal() as s:
            row=s.get(TrackRecord,{"id":track_id,"user_id":self.user_id}); return self._track(row) if row else None
    def set_custom_lyrics(self, track_id: str, lyrics: str):
        with SessionLocal() as s:
            row=s.get(TrackRecord,{"id":track_id,"user_id":self.user_id})
            if not row:return None
            row.custom_lyrics=lyrics;s.commit();return self._track(row)
    ...
    def cover_jpeg(self, track_id: str, size: int=512):
        ...
        if not track:return b""
        image=Image.open(io.BytesIO(track.cover_data)).convert("RGB").resize((size,size),Image.LANCZOS);out=io.BytesIO();image.save(out,format="JPEG",quality=90);return out.getvalue()
